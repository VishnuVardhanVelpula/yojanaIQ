...
LANGUAGE, AGE, GENDER, CASTE, RELIGION, OCCUPATION, INCOME, FLAGS, CHAT = range(9)
...
BOT_I18N = {
    "English": {
        ...
        "occ_opts": [["Student", "Farmer"], ["Street Vendor", "Small Business"], ["Unemployed", "Salaried"], ["Other"]],
        "flag_opts": [["Widow", "Disabled"], ["SHG Member", "Houseless"], ["None of these"]]
    },
    "Telugu": {
        ...
        "occ_opts": [["విద్యార్థి", "రైతు"], ["వీధి వ్యాపారి", "చిన్న వ్యాపారం"], ["నిరుద్యోగి", "ఉద్యోగి"], ["ఇతర"]],
        "flag_opts": [["వితంతువు", "వికలాంగుడు"], ["SHG సభ్యుడు", "ఇల్లు లేని వారు"], ["ఏవీ కావు"]]
    },
    "Hindi": {
        ...
        "occ_opts": [["छात्र", "किसान"], ["सड़क विक्रेता", "छोटा व्यवसाय"], ["बेरोजगार", "वेतनभोगी"], ["अन्य"]],
        "flag_opts": [["विधवा", "विकलांग"], ["SHG सदस्य", "बेघर"], ["इनमें से कोई नहीं"]]
    }
}
...
async def get_flags(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        flag_map = {
            "widow": "widow", "disabled": "disabled",
            "shg_member": "shg_member", "houseless": "houseless",
            "none_of_these": None
        }
...
async def handle_inline_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lang = context.user_data.get("language", "English")
    msg_wait = "⏳ Processing details for *" + scheme_name + "*..."
    if lang == "Telugu": msg_wait = f"⏳ *{scheme_name}* వివరాలను పొందుతోంది..."
    elif lang == "Hindi": msg_wait = f"⏳ *{scheme_name}* का विवरण प्राप्त किया जा रहा है..."

    await query.message.reply_text(msg_wait, parse_mode="Markdown")

    profile = context.user_data["profile"]
    
    prompt = f"Tell me everything about {scheme_name}"
    
    try:
        result = run_rag(profile, user_query=prompt, language=lang)
